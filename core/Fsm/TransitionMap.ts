...
namespace Truth
{
	...
	export class TransitionMap
	{
		...
		{
			const transitions = new Map<number, TransitionState>();
			...
			if (transitionLiteral)
			{
				for (const [stateIdText, tslObject] of Object.entries(transitionLiteral))
				{
					...
					const tsl: ITransitionStateLiteral = tslObject;
					transitions.set(stateId, new TransitionState(tsl));
				}
			}
		...
		*[Symbol.iterator]()
		{
			for (const [stateId, transitionState] of this.transitions.entries())
				yield <[number, TransitionState]>[stateId, transitionState];
		}
	...
	export class MutableTransitionMap extends TransitionMap
	{
		...
		{
			this.transitions.set(srcStateId, new TransitionState());
		}
		...
		set(srcStateId: number, symbol: string, dstStateId: number)
		{
			...
			if (!tState)
			{
				const tState = new TransitionState();
