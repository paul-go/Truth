...
namespace Truth
{
	 ...
	 * 
	 */
	export function parseUri(uriText: string): Partial<Uri> | null
	{
		let isRelative = false;
		let retractionCount = 0;
		let protocol = "";
		
		const parser = new Parser(uriText);
		...
		function maybeReadCurrent()
		{
			...
			if (parser.read(UriSyntax.current))
				if (parser.read(UriSyntax.componentSeparator))
					return true;
		...
		{
			const token = UriSyntax.retract + UriSyntax.componentSeparator;
		...
		function maybeReadProtocol()
		{
			...
			if (parser.read(UriSyntax.protocolRelative))
			{
				protocol = UriProtocol.unknown;
				return true;
			}
		...
		function readComponents(side: "store" | "type")
		{
			...
			while (parser.more())
			{
				...
				if (side === "store" && parser.read(UriSyntax.typeSeparator))
					break;
		...
		function readComponent()
		{
			...
			while (parser.more())
			{
				if (parser.peek(UriSyntax.componentSeparator))
					break;
			...
			return new UriComponent(port ? value + portChar + port : value);
		}
		...
		function maybeReadAnonymousComponent()
		{
			...
			let anon = parser.read(UriSyntax.indexerStart);
			if (!anon)
				return null;
				
			while (parser.more())
			{
				if (parser.read(UriSyntax.indexerEnd))
					return anon.length > 0 ?
						new UriComponent(anon + UriSyntax.indexerEnd) :
						cancel();
		...
		else if (parser.read(UriSyntax.componentSeparator))
		{
		...
		if (parser.readThenTerminal(UriSyntax.componentSeparator))
			return null;
		
		// URI ends with a //, this isn't a valid URI
		if (parser.readThenTerminal(UriSyntax.typeSeparator))
			return null;
		...
		const ext = (() =>
		{
			if (file.endsWith(UriExtension.truth))
				return UriExtension.truth;
			
			if (file.endsWith(UriExtension.js))
				return UriExtension.js;
			
			if (file.endsWith(UriExtension.wasm))
				return UriExtension.wasm;
			
			return UriExtension.unknown;
		})();
		...
		if (ext !== UriExtension.unknown)
			stores.pop();
		
		return {
			protocol: UriProtocol.resolve(protocol) || UriProtocol.unknown,
			file: ext ? file : "",
			ext,
			retractionCount,
			isRelative,
