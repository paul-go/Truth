...
namespace Truth
{
	...
	export class DocumentGraph
	{
		...
		constructor(program: Program)
		{
			...
			{
				if (data.uri.ext !== UriExtension.truth)
					return;
		 ...
		 */
		async read(uri: string | Uri)
		{
			const uriParsed = Uri.tryParse(uri);
			if (!uriParsed)
		...
		create(param1?: Uri | string, param2?: string)
		{
			...
			{
				if (zeroArgs || oneArg)
					return Uri.createInternal();
				
				if (!param1)
					return null;
				
				if (param1 instanceof Uri)
					return param1;
				
				return Uri.tryParse(param1);
			})();
			...
			const document = new Document(this.program, uri, sourceText);
			const header = new DocumentHeader(document);
			const entry: IDocumentEntry = { document, header };
			this.documents.set(uri.toStoreString(), entry);
			header.recompute();
			
			this.program.cause(new CauseDocumentCreate(document));
			return document;
		}
		...
		each()
		{
			...
			const sortedResult: Document[] = [];
			const docsFinalized = new Set<Document>();
			const docsInStack = new Set<Document>();
			const allDocs = Array.from(this.documents.values())
				.map(entry => entry.document);
			
			const recurse = (currentDoc: Document) =>
			{
		 ...
		 */
		delete(target: Document | Uri)
		{
			const doc = target instanceof Document ? target : (() =>
			{
			...
			this.program.cause(new CauseDocumentCreate(doc));
		...
		getDependencies(doc: Document)
		{
			...
			const depsSorted = urisSorted.map(uriText => 
				Not.undefined(docDependencies.find(docDep => 
			...
			uri: Uri)
		{
			...
			setTimeout(() => (async () =>
			{
				const refDocument = await (async () =>
				{
					...
					{
						const fault = Faults.UnresolvedResource.create(containingStatement);
				...
				if ((srcProto === UriProtocol.http || srcProto === UriProtocol.https) && 
					dstProto === UriProtocol.file)
				{
					const param = Faults.InsecureResourceReference.create(containingStatement);
				...
				{
					const param = Faults.CircularResourceReference.create(containingStatement);
					this.program.faults.reportAsync(param);
				}
				else
				{
		 ...
		 * relationship.
		 */
		private wouldCreateCycles(proposedFrom: Document, proposedTo: Document)
		{
			const checkForCyclesRecursive = (current: Document) =>
			{
		 ...
		 */
		private link(from: Document, to: Document)
		{
		 ...
		 */
		private unlink(from: Document, to: Document)
		{
			const dependencyArray = this.dependencies.get(from);
			if (!dependencyArray)
				return;
		 ...
		 */
		private readonly dependencies = new Map<Document, Dependency[]>();
		
		/**
		 * A map of the documents that depend on each document.
		 */
		private readonly dependents = new Map<Document, Document[]>();
		...
		toString()
		{
			...
			{
				const uri = Uri.tryParse(uriText);
	...
	class Dependency
	{
		constructor(readonly target: Document) { }
