...
namespace Truth.Test
{
	...
	{
		const cases = Object.entries(Acceptance);
		
		test.each(cases)(`Parse "%s"`, (source: string, expected: IExpectation) =>
		{
			const parsedLine: Line = Not.undefined((() =>
			{
				let out: Line | null = null;
				
				try
				{
					out = LineParser.parse(source);
				}
				catch (e)
				{
					debugger;
