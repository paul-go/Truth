...
namespace Truth.Test
{
	...
	expect.extend({
		 ...
		 */
		toRead(actual: Statement[], expected: string)
		{
			...
			if (actual.some(s => !(s instanceof Statement)))
		...
		toHaveFaults(
			program: Program,
			...expectations: [number, Statement | Span][])
		{
			...
			const sortedExpectations = expectations.slice().sort((a, b) =>
			{
				...
				const faultName1 = Faults.nameOf(a[0]);
				const faultName2 = Faults.nameOf(b[0]);
			...
			{
				if (a.source instanceof Statement || a.source instanceof Span)
					if (b.source instanceof Statement || b.source instanceof Span)
			...
			{
				const expType = <typeof Fault>sortedExpectations[i][0];
				const actType = <typeof Fault>sortedActuals[i].constructor;
