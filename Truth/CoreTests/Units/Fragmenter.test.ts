...
describe("Fragmenter Tests", () =>
{
	...
	test("1-Level + No Fragmentation", () =>
	{
		...
		expect(fragContent).toBe(T.outdent`
	...
	// Skipping this test because it doesn't test
	// the current behavior of the fragmenter.
	test.skip("Fragmenter Query", () =>
	{
		const prog = new X.Program();
		const doc = prog.documents.create(T.outdent`
			A : 1
		...
		expect(statements).toRead(T.outdent`
			C : 3
			C : 4
			C : 6
			C : 7
